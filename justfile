...
containerized-integration-test project:
    ...
    for test in "${tests[@]}"; do
        ...
        podman run --rm --interactive --tty --volume "$test_tmpdir:/mnt" --volume "$PWD:/test" --volume "$target_dir/debug:/usr/local/bin" arch-signstar-integration-test cargo nextest run --nocapture --archive-file "/mnt/tests.tar.zst" --workspace-remap "/test" "${test/ */}" "${test/* /}"
    done
