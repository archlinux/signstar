...
use signstar_crypto::key::{KeyMechanism, KeyType, PrivateKeyImport, key_type_matches_length};
...
impl<'a> HsmKey<'a> {
    ...
    fn new(raw_signer: &'a dyn RawSigner, public_key: PublicKey) -> Self {
        Self {
            raw_signer,
            public_key,
        }
    ...
    fn new_provisioned(raw_signer: &'a dyn RawSigner) -> Result<Self, Error> {
...
impl pgp::types::KeyDetails for HsmKey<'_> {
...
pub fn add_certificate(
    ...
    let signer = HsmKey::new(raw_signer, public_key.clone());
...
///
/// Signs the message `message` using the [`RawSigner`] and returns a binary [OpenPGP data
/// signature].
///
/// # Errors
///
...
pub fn sign(raw_signer: &dyn RawSigner, message: &[u8]) -> Result<Vec<u8>, Error> {
    let signer = HsmKey::new_provisioned(raw_signer)?;
...
///
/// Signs the hasher `state` using the [`RawSigner`] and returns a binary [OpenPGP data signature].
///
/// # Errors
///
/// Returns an [`Error`] if creating an [OpenPGP signature] for the hasher state fails:
...
/// [role]: https://docs.nitrokey.com/nethsm/administration#roles
/// [state]: https://docs.nitrokey.com/nethsm/administration#state
pub fn sign_hasher_state(raw_signer: &dyn RawSigner, state: sha2::Sha512) -> Result<String, Error> {
    let signer = HsmKey::new_provisioned(raw_signer)?;
