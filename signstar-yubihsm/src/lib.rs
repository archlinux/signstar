...
pub struct Credentials {
    ...
    pub passphrase: String, // FIXME: this should be Passphrase
}

impl From<&Credentials> for YubiCredentials {
    fn from(value: &Credentials) -> Self {
        YubiCredentials::from_password(value.auth_key_id, value.passphrase.as_bytes())
    }
}
...
impl YubiHsmSigner {
    ...
    pub fn mock(key_id: u16, credentials: &Credentials) -> Result<Self, Error> {
        ...
        let auth_key = authentication::Key::derive_from_password(credentials.passphrase.as_bytes());
