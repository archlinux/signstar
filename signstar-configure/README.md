...
- one or more HSM backends (e.g. [NetHSM] and/or [YubiHSM2]) connected to a _Signstar host_,
...
As a backup, the hardcoded default connection - if applicable - of an unprovisioned HSM backend is checked.
...
**TODO**: Figure out how to map iterations to the state of an HSM backend.
...
First restore any backend from backup, that is in _unprovisioned_ state, then - on a best effort basis - attempt to restore from backup any backend that has already been in _provisioned_ state by iteratively trying the provided administrative credentials.
