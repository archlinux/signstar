    ...
    pub fn sync_yubihsm2(&self) -> Result<ConfigurationResult, Error> {
        ...
        let available_connections = {
            ...
            if available_connections.is_empty() {
                ...
                return Ok(ConfigurationResult::NoAvailableBackendConnection);
            }
        ...
        let admin_credentials = match self.load_yubihsm2_admin_credentials() {
            ...
            Err(Error::SignstarConfig(signstar_config::Error::AdminSecretHandling(
                ...
                if !provisioned_backends.is_empty() {
                    error!(
                        "There are provisioned backends, but administrative credentials are not present. You probably want to restore from a backup. Aborting..."
                    );
                    return Ok(ConfigurationResult::ProvisionedBackendsButNoAdminCreds);
                }
                if available_connections.len() != yubihsm2_config.connections().len() {
                    ...
                    return Ok(ConfigurationResult::NotAllConnectionsAvailableAndNoAdminCreds);
                }
