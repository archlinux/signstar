...
fn get_tmp_config(data: &[u8]) -> Result<NamedTempFile, Error> {
...
fn write_machine_id() -> Result<(), Error> {
...
fn start_credentials_socket() -> Result<BackgroundProcess, Error> {
...
pub fn run_command_as_user(
    ...
    //    let command = command.spawn().expect("spawn to succeed");
    ...
    command_output
        ...
        .take()
        .unwrap()
        .write_all(&command_input)
        .unwrap();
    let exit = command_output.wait().unwrap();
    let mut stdout = String::new();
    BufReader::new(command_output.stdout.take().unwrap())
        .read_to_string(&mut stdout)
        .unwrap();
    eprintln!("stdout:\n{stdout}");

    let mut stderr = String::new();
    BufReader::new(command_output.stderr.take().unwrap())
