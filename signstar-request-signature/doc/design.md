...
This document describes several technical details of the current design.
...
sequenceDiagram
    ...
    S ->> S: create PGP packets and append them to the hasher
    S ->> S: finalize the PGP hasher stream
...
Here, the internal hasher state is transmitted instead of the final digest.

There are several reasons to do so:

First and foremost it decouples the act of hashing a file from the framing that is imposed by the target cryptographic system (e.g. [OpenPGP][9580]).

In OpenPGP v4 signatures the final digest to be signed is calculated by first hashing the file and then [concatenating OpenPGP packets][SD] as if they were appended at the end of the file.
Transmitting the state makes it possible for the client to do just the file hashing with no OpenPGP handling at all.
The OpenPGP bits are appended by the server.

This unlocks another opportunity: The server may append any packets that it sees fit, for example timestamping notations, capturing the identity of the signer or any constraints on the signature (such as making the signatures expire).

Yet another advantage: Since the state is just an array of bytes the signing server may additionally finalize it early to recover the digest of the file to be signed.
In general, this is not possible with just an OpenPGP signature as the fields in the signature mention the digest of already concatenated data (file + OpenPGP packets) data.
Our implementation inserts a `data-digest@archlinux.org` [notation][NOTD] with the digest of the file that was hashed by the client.
The digest algorithm used is [already part of the data signature][SIGS].

[NOTD]: https://www.rfc-editor.org/rfc/rfc9580#name-notation-data

Since it is the server appending notations, the client cannot fake them.
This puts more pressure on the signing server to be trustworthy (as the client sends a state a malicious server could misuse the state to claim a signature over a different file).
This is an acceptable trade-off and sane assumption to make, as a server already provides sensitive operations (using private key material) either way.

This design is easily expandable to other systems such as SSH signatures, which require the file digest (the server can finalize the state early).
...
Currently, there are two types of signatures: Version 4 and version 6.
...
Signstar's signing request model relies on the transmission of hasher state.
This enables a model in which the client only computes the digest of the file to be signed, but for no additional components used in a cryptographic context.

The server creates `pgp-metadata` and continues the hashing, which produces the final `digest-to-be-signed`.

As hasher state is transmitted, the server is able to recover the digest of the file, which we embed in the OpenPGP metadata as a signature notation.

Unfortunately this method works only for V4 signatures.
Version 6 signatures prepend the randomly generated "salt" value which precludes calculation of the file digest by the server.
This still allows us to issue a signature over `salt file` combination, though.

Note that a point has been raised that transmitting the state is ["well beyond the typical analysis"][SA].
...
flowchart TD
    ...
    WRAP(wrap in a signerInfo PKCS#7 container) --> WRAP2
    WRAP2(wrap in a signedData PKCS#7 container) --> DIGEST2
    DIGEST2(compute digest) --> DTS
    DTS[data to be signed] --> SIG
    SIG(update signerInfo inserting the actual signature value)
```
