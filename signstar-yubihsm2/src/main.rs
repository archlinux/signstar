...
use signstar_yubihsm2::{Error, runner::ScenarioRunner, scenario::Scenario};
use yubihsm::{Connector, UsbConfig};
...
fn run_scenario(serial_number: Option<String>, scenario_file: PathBuf) -> Result<(), Error> {
    let connector = if let Some(serial_number) = serial_number {
        Connector::usb(&UsbConfig {
            serial: Some(serial_number.parse().map_err(|source| Error::Device {
                context: "parsing YubiHSM2 serial number",
                source,
            })?),
