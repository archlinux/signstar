...
- **shamirs-secret-sharing**: This object describes the encryption and decryption of administrative credentials using **Shamir's Secret Sharing**[3].
  In this scheme, several individuals are provided with cryptographic shares of the (encrypted) administrative credentials.
  A subset of these individual's cryptographic shares are required to decrypt the administrative credentials.
...
- **tls\_security**: The TLS security used for the **url**.
  ...
  This must be one of the following two strings, or the **root\_certificates** object:
    - **unsafe**: Always trust the TLS certificate associated with a host.
      ...
      **WARNING**: This does not check the host key!
...
- **mock**: This string enables the use of an emulated, in-memory HSM for testing purposes.
  ...
  **WARNING**: This option should only be used for testing purposes and it is only available, if YubiHSM2 mockhsm support is compiled in.
...
- **domain**: The unique domain, that the **authentication\_key\_id** and **signing_key_id** are part of.
  Domains partition the access of individual authentication key IDs on the device.
...
**nethsm**(1), **signstar-sign**(1), **signstar-configure**(1), **signstar-configure-build**(1), **systemd-creds**(1)
