...
use rstest::rstest;
use signstar_common::{common::get_data_home, config::get_default_config_file_path};
...
/// Recursively lists files, their permissions an ownership.
...
fn load_systemd_creds(#[case] config_data: &[u8]) -> TestResult {
    write_machine_id()?;
    let config_file_fixture = get_tmp_config(config_data)?;
    ...
    for mapping in &creds_mapping {
        if let Some(system_user_id) = mapping.get_user_mapping().get_system_user() {
            ...
            run_command_as_user(&[&get_creds_bin], system_user_id.as_ref())?;
        } else {
