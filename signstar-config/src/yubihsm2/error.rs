use signstar_yubihsm2::yubihsm::Id;

use crate::yubihsm2::admin_credentials::YubiHsm2AdminCredentials;

/// An error that may occur when using YubiHSM2 config objects.
#[derive(Debug, thiserror::Error)]
pub enum Error {
    /// There is no usable administrative user on the backend.
    #[error(
        "There is no usable administrative user in the YubiHSM2 backend. It needs to be reset."
    )]
    NoUsableAdmin,

    /// The default administrative credentials are still usable in the backend.
    #[error(
        "The default administrative credentials ({}: {}) are still usable on the YubiHSM2 backend.",
        YubiHsm2AdminCredentials::DEFAULT_ID,
        YubiHsm2AdminCredentials::DEFAULT_PASSPHRASE
    )]
    DefaultAdminStillUsable,
