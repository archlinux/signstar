...
/// used. They can be derived e.g. Signstar backends or Signstar configuration files.
...
/// The type of a state in a Signstar configuration file.
#[derive(Clone, Copy, Debug, strum::Display, Eq, PartialEq)]
pub enum SignstarConfigStateType {
...
impl From<SignstarConfigStateType> for StateType {
    fn from(value: SignstarConfigStateType) -> Self {
        match value {
            SignstarConfigStateType::NetHsm => Self::SignstarConfigNetHsm,
            SignstarConfigStateType::YubiHsm2 => Self::SignstarConfigYubiHsm2,
        }
    }
}
