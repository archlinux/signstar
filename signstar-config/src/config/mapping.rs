...
impl UserMapping {
    ...
    ///
    /// ```
    /// use signstar_config::{AuthorizedKeyEntryList, SystemUserId, UserMapping};
    ///
    /// # fn main() -> testresult::TestResult {
    ...
    /// ```
    /// use nethsm::UserId;
    /// use signstar_config::{AuthorizedKeyEntryList, UserMapping};
    ///
    /// # fn main() -> testresult::TestResult {
    /// let mapping = UserMapping::SystemOnlyShareDownload {
    ///     system_user: "user1".parse()?,
    ...
    ///
    /// ```
    /// use signstar_config::{AuthorizedKeyEntry, AuthorizedKeyEntryList, UserMapping};
    ///
    /// # fn main() -> testresult::TestResult {
    /// let mapping = UserMapping::SystemOnlyShareDownload {
    ///     system_user: "user1".parse()?,
    ...
    /// use signstar_config::{AuthorizedKeyEntryList, FilterUserKeys, UserMapping};
    ///
    /// # fn main() -> testresult::TestResult {
    /// let mapping = UserMapping::SystemNetHsmOperatorSigning {
    ///     nethsm_user: "user1".parse()?,
                ...
                match secret_handling {
                    ...
                    NonAdministrativeSecretHandling::SystemdCreds => {
                        ...
                        let Some(mut stdin) = command_child.stdin.take() else {
                            return Err(Error::CommandAttachToStdin {
                                command: format!("{command:?}"),
                            })?;
                        };
                        ...
                        let _handle_result = handle.join().map_err(|source| Error::Thread {
                            context: format!(
