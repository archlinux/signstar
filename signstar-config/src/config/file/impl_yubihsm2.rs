//! Impls for [`UserBackendConnection`] and [`Config`] when only using a YubiHSM2 backend.
//!
//! # Note
//!
//! This module with `impl` blocks is only used, if the `yubihsm2` feature, but otherwise no other
//! HSM backend is enabled.
