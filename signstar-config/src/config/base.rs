...
use std::{collections::HashSet, error::Error as StdError, path::Path};
...
impl SignstarConfig {
    /// Creates a new [`SignstarConfig`] from a configuration file.
    ///
    /// # Errors
    ///
    ...
    pub fn new_from_file(path: Option<&Path>) -> Result<Self, crate::Error> {
        let config: SignstarConfig = if let Some(path) = path {
            confy::load_path(path).map_err(|error| Error::ConfigFailedToLoad {
            ...
            confy::load("signstar", Some("signstar")).map_err(|error| {
                Error::ConfigFailedToLoad {
                    description: if let Some(error) = error.source() {
                        error.to_string()
                    } else {
                        "".to_string()
    ...
    pub fn store(&self, path: Option<&Path>) -> Result<(), crate::Error> {
        if let Some(path) = path {
            // confy::store_path(path, self).map_err(|_source| crate::Error::from(Error::Store))
            confy::store_path(path, self)
                .map_err(|source| crate::Error::Config(Error::ConfigFailedToStore(source)))
        } else {
            confy::store("signstar", "config", self)
                .map_err(|source| crate::Error::Config(Error::ConfigFailedToStore(source)))
        }
    }
    ...
    pub fn get_extended_mapping_for_user(
        &self,
        name: &str,
    ) -> Result<ExtendedUserMapping, crate::Error> {
