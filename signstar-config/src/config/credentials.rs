...
impl AuthorizedKeyEntryList {
    ...
    ///
    /// ```
    /// use signstar_config::AuthorizedKeyEntryList;
    ///
    /// # fn main() -> testresult::TestResult {
    ...
    /// # }
    /// ```
    pub fn new(ssh_authorized_keys: Vec<AuthorizedKeyEntry>) -> Result<Self, crate::Error> {
        if ssh_authorized_keys.is_empty() {
            return Err(ConfigError::NoAuthorizedKeys.into());
        }
            ...
            .collect::<Vec<(AuthorizedKeyEntry, PublicKey)>>()
        {
            if !set.insert(pub_key) {
                return Err(ConfigError::DuplicateSshPublicKey {
                    ssh_public_key: ssh_authorized_key.to_string(),
                }
                .into());
            }
        }
