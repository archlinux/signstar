...
impl<'a, 'b> NetHsmBackend<'a, 'b> {
    ...
    pub(crate) fn nethsm(&self) -> &NetHsm {
        &self.nethsm
    }
    ...
    pub(crate) fn user_states(&self) -> Result<Vec<crate::config::state::UserState>, crate::Error> {
        // Use the default R-Administrator.
        self.nethsm
            .use_credentials(&self.admin_credentials.get_default_administrator()?.name)?;

        let users = {
            let mut users: Vec<crate::config::state::UserState> = Vec::new();
            ...
            for user_id in self.nethsm.get_users()? {
                ...
                users.push(crate::config::state::UserState {
                    name: user_id,
            ...
            Ok(Some(key_cert)) => {
                let public_key = match SignedPublicKey::from_reader_single(key_cert.as_slice()) {
                    ...
                    Err(error) => {
                        ...
                        debug!("{message}");
                        return crate::config::state::KeyCertificateState::NotAnOpenPgpCertificate { message };
                    }
                };

                match TryInto::<CryptographicKeyContext>::try_into(public_key) {
                    Ok(key_context) => {
                        crate::config::state::KeyCertificateState::KeyContext(key_context)
                    }
                    Err(error) => {
                        let message = format!(
                        ...
                        debug!("{message}");
                        crate::config::state::KeyCertificateState::NotACryptographicKeyContext {
                            message,
                        }
                    }
                }
            }
            Ok(None) => crate::config::state::KeyCertificateState::Empty,
            Err(error) => {
                let message = error.to_string();
                debug!("{message}");
                crate::config::state::KeyCertificateState::Error { message }
            }
        }
    }
    ...
    pub(crate) fn key_states(&self) -> Result<Vec<crate::config::state::KeyState>, crate::Error> {
        // Use the default administrator
        ...
        for key_id in self.nethsm.get_keys(None)? {
            ...
            keys.push(crate::config::state::KeyState {
                name: key_id,
                namespace: None,
