...
//! # Note
//!
//! This module only works with data for the same iteration (i.e. the iteration of administrative
//! credentials and those of the NetHSM backend must match).
...
/// Adds all users in the system-wide [`UserRole::Administrator`] role to a [`NetHsm`].
///
...
fn add_system_wide_admins(
    ...
    info!("Setting up system-wide administrators.");
...
/// Returns a list of available users in the [`Administrator`][`UserRole::Administrator`] role in
/// a namespace.
///
/// Derives a list of `namespace` users in the [`Administrator`][`UserRole::Administrator`] role
...
/// Returns an error if
///
/// - user information of a user in the [`Administrator`][`UserRole::Administrator`] role cannot be
///   retrieved,
/// - or no user in the [`Administrator`][`UserRole::Administrator`] is available in the
///   `namespace`.
...
/// Adds all users in the role of a namespace [`Administrator`][`UserRole::Administrator`] and their
...
/// - one of the administrator users in the `admin_credentials` is not in a namespace,
/// - retrieving the list of namespace administrators
fn add_namespace_admins(
    nethsm: &NetHsm,
    admin_credentials: &AdminCredentials,
) -> Result<(), crate::Error> {
    info!("Setting up namespace administrators");
    ...
    for user in admin_credentials.get_namespace_administrators() {
        ...
        if !available_users.contains(&user.name) {
            ...
            if available_namespaces.contains(namespace) {
                // Retrieve all namespace admins currently available on the NetHSM backend.
...
fn add_non_administrative_users(
    ...
    info!("Setting up system-wide non-administrative users");
    ...
    for mapping in users.iter() {
        ...
        for (user, role) in mapping
            .get_nethsm_users_and_roles()
            .iter()
            // Only use system-wide, non-administrative users
            .filter(|(user, role)| !user.is_namespaced() && role != &UserRole::Administrator)
        {
...
fn add_namespaced_non_administrative_users(
    ...
    info!("Setting up namespaced non-administrative users");
...
fn add_system_wide_keys(
    ...
    info!("Setting up system-wide keys");
...
fn add_namespaced_keys(
    ...
    info!("Setting up namespaced keys");
...
fn add_system_wide_openpgp_certificates(
    ...
    info!("Setting up system-wide OpenPGP certificates");
...
fn add_namespaced_openpgp_certificates(
    ...
    info!("Setting up namespaced OpenPGP certificates");
...
fn provision(nethsm: &NetHsm, admin_credentials: &AdminCredentials) -> Result<(), crate::Error> {
    info!("Provisioning NetHSM: {nethsm:?}");
    nethsm.provision(
...
impl<'a, 'b> FullNetHsmBackend<'a, 'b> {
    ...
    pub fn state(&self) -> Result<NetHsmState, crate::Error> {
        info!("Create state for NetHSM connection {:?}", self.nethsm);
    ...
    pub fn sync(&self, user_credentials: &[FullCredentials]) -> Result<(), crate::Error> {
        ...
        match self.nethsm.state()? {
            SystemState::Unprovisioned => {
                info!("Unprovisioned backend detected");
                provision(&self.nethsm, self.admin_credentials)?;
            }
            // If the backend is locked, unlock it and sync
            SystemState::Locked => {
                info!("Locked backend detected");
                self.nethsm.unlock(Passphrase::new(
                    self.admin_credentials.get_unlock_passphrase().into(),
                ))?;
            }
            SystemState::Operational => {
                info!("Operational backend detected");
            }
        }
