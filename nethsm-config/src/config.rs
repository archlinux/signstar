...
pub enum Error {
    ...
    /// A requested system user name is not found in a configuration.
    #[error("No mapping with a system user named {0} found")]
    NoMatchingSystemUser(String),
...
impl HermeticParallelConfig {
    ...
    pub fn get_extended_mapping_for_user(&self, name: &str) -> Result<ExtendedUserMapping, Error> {
        ...
        Err(Error::NoMatchingSystemUser(name.to_string()))
    }
