...
impl UserMapping {
    ...
    /// Returns [`UserId`], [`SigningKeySetup`] and tag of the mapping
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// };
    /// assert_eq!(
    ///     mapping.get_user_key_and_tag(FilterUserKeys::All),
    ///     vec![(
    ///         UserId::new("user1".to_string())?,
    ///         SigningKeySetup::new(
    ///             "key1".parse()?,
    ...
    ///     )]
    /// );
    /// assert_eq!(mapping.get_user_key_and_tag(FilterUserKeys::Namespace("test".parse()?)), vec![]);
    /// assert_eq!(mapping.get_user_key_and_tag(FilterUserKeys::Tag("tag2".parse()?)), vec![]);
    ///
    /// let mapping = UserMapping::SystemOnlyShareDownload {
    ///     system_user: "user1".parse()?,
    ///     ssh_authorized_keys: AuthorizedKeyEntryList::new(vec!["ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIH3NyNfSqtDxdnWwSVzulZi0k7Lyjw3vBEG+U8y6KsuW user@host".parse()?])?,
    /// };
    /// assert_eq!(mapping.get_user_key_and_tag(FilterUserKeys::All), vec![]);
    /// # Ok(())
    /// # }
    /// ```
    pub fn get_user_key_and_tag(
        &self,
